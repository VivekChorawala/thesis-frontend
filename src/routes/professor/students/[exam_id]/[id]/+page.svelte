<script lang="ts">
	...
	const { getResultFunc } = exams();

	let exam_id = $page.params.exam_id;
	let student_id = $page.params.id;

	let selectedOption: any, loading: any = false;
	let studentResult: any = [];
	$: studentResult, selectedOption;
	const getResult = async () => {
		let apiCall = await getResultFunc(exam_id, student_id);

		if (apiCall.status == 'success') {
			studentResult = apiCall.data;

			studentResult.questions.forEach((element: any, i) => {
				let selectedOptionIndex = element.option.findIndex(
					(opt: any) => opt.id === element?.answer?.option_id?.id
				);
						...
						{#each studentResult.questions as question, i}
							<div class="question {i != 0 ? 'mt-2' : ''} mb-2">
									...
									{#each question.option as opt, index}
										<div class="flex items-center gap-2 py-2">
											<input
												...
												id="option.{opt.id}"
												value={opt.id}
												checked={question.answer && opt.id == question.answer.option_id.id
													? 'checked'
													: ''}
											/>
											{opt.option}
								...
								<p class="font-medium text-black">
									...
									question.option.find((item) => item.id === question.answer.option_id.id).answer
										? 1
										: 0}
								</p>
							</div>
