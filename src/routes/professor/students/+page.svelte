<script lang="ts">
    ...
    import { Table, TableBody, TableBodyCell, TableBodyRow, TableHead, TableHeadCell, TableSearch, Alert,  Spinner } from 'flowbite-svelte';
    import moment from 'moment';
	import {  InfoCircleSolid , InfoCircleOutline } from 'flowbite-svelte-icons';
	import { goto } from '$app/navigation';
	  
    let studenExamList: any = [];
    let searchTerm = '', user: any;
    $: studenExamList, user;
    ...
    let { getUser, professorExamStudentListing } = professorExams();
    const getExamsByProfessor = async() => {
      
      let apiCall = await professorExamStudentListing();
      
      if(apiCall.status == 'success') {
        filteredItems = await apiCall.data;  
        studenExamList = apiCall.data;
        if(studenExamList.length > 0){
          studenExamList = studenExamList.filter((item: any) => item.student_exam_finished_at != null);
        }
      } else {
        let role = user.role;
    ...
    $: filteredItems = studenExamList.filter((item: any) => {
  ...
  {#if loading == false}
	<div class="mx-auto max-w-8xl sm:px-6 lg:px-8 bg-white p-4 mt-10">
      <div class="flex items-center justify-between gap-x-6 py-4">
        <h1 class="text-3xl font-bold leading-tight tracking-tight text-gray-900">Professor Exams Student List</h1>
      </div>
        {#if showAlert}
				<div>
        ...
        <Table>
            ...
            <TableBody class="divide-y">
                ...
                <TableBodyRow> 
                    ...
                    <TableBodyCell>{moment(item.student_exam_start_time).format('YYYY-MM-DD HH:mm:ss A')}</TableBodyCell>
                    <TableBodyCell>{moment(item.student_exam_start_time).format('YYYY-MM-DD HH:mm:ss A')}</TableBodyCell>
                    <TableBodyCell>{item.subject_id}</TableBodyCell>
                    <TableBodyCell>{item.subject_name}</TableBodyCell>
                    <TableBodyCell>{moment(item.exam_start_time).format('YYYY-MM-DD HH:mm:ss A')}</TableBodyCell>
                    <TableBodyCell>{moment(item.exam_finish_time).format('YYYY-MM-DD HH:mm:ss A')}</TableBodyCell>
                    ...
                    <TableBodyCell>
                        <div class="flex items-center gap-2	">
                            <a href="/professor/students/{item.exam_id}/{item.student_id}" class="font-medium text-primary-500 hover:underline dark:text-primary-400">
