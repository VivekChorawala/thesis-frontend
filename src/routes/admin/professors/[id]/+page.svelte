...
let { getOneProfessor } = professors();
let userid = $page.params.id, loading: any = false;
let user: any = {};
$: user;
const getIndividualUser = async() => {
  
  let apiCall = await getOneProfessor(Number(userid));
...
<header>
	<div class="mx-auto max-w-8xl px-4 sm:px-6 lg:px-2 lg:mt-2">
		<h1 class="text-3xl font-bold leading-tight tracking-tight text-gray-900">Show Professor</h1>
	</div>
</header>
<main>
  {#if loading == false}
    {#if user != null}
      <div class="mx-auto max-w-8xl sm:px-6 lg:px-8 bg-white p-4">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
          <div class="mb-6">
            <Label for="email" class="mb-2">Subjects</Label>
            <p>{user.subject}</p>
          </div>
        </div>
      </div>
