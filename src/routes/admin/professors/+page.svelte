<script lang="ts">
	...
	import { professors } from '../../api/admin/professors';
	let openRow:number | null;
	let searchTerm = '';
	let defaultModal: any = { open: false, userid: null};
  	let { getAllProfessors, updateStatusUser, deleteUser } = professors();
	let userList: any = [];
	$: userList;
	...
	async function getProfessors() {
		let apiCall = await getAllProfessors();
...
<main class="container">
	{#if loading == false}
		<div class="mx-auto max-w-8xl sm:px-6 lg:px-8 bg-white p-4 mt-10">
			<h1 class="text-3xl font-bold leading-tight tracking-tight text-gray-900 py-4">Professors</h1>
			{#if showAlert}
				...
				<Table>
					...
					<TableBody tableBodyClass="divide-y divide-gray-100">
						...
						<TableBodyRow on:click={() => toggleRow(i)}>
							...
							<TableBodyCell>
								<!-- <a href="/admin/professors/{item.id}" class="font-medium text-primary-500 hover:underline dark:text-primary-400">Show</a>  -->
								...
								<div class="flex items-center gap-2">
									<a href="/admin/professors/{item.id}" class="font-medium text-primary-500 hover:underline dark:text-primary-400">
		...
		{:else}
			<div class="h-32 flex items-center justify-center">No data found</div>
		{/if}
				<!-- <Modal title="Delete Professor" bind:open={defaultModal.open} autoclose>
					<p class="text-base text-center leading-relaxed text-gray-500 dark:text-gray-400">Are you sure, you want to delete selected professor.</p>
				...
				<Modal title="Delete Professor" bind:open={defaultModal.open} size="xs" autoclose>
					<div class="text-center">
						<ExclamationCircleOutline class="mx-auto mb-4 text-gray-400 w-12 h-12 dark:text-gray-200" />
						<h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure, you want to delete selected professor?</h3>
